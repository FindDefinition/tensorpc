...
def _attn_fwd_kernel_test_fn(N_CTX: int = 256, HEAD_DIM: int = 64, H = 1, dtype = torch.float32, is_fwd: bool = True, head_first: bool = True) -> pfl.PFLInlineRunEnv:
...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        acc = acc * alpha[:, None]
        # update acc
        v = tl.load(V_block_ptr, cache_modifier=".cg")
        if fp8_v:
            p = p.to(tl.float8e5)
        else:
            p = p.to(q.dtype)
...
def _flash_triton_bench_fn_bwd(kwargs):
    from flash_attn.flash_attn_triton import flash_attn_func
    q = kwargs["Q"].detach().clone().transpose(1, 2).contiguous()
    k = kwargs["K"].detach().clone().transpose(1, 2).contiguous()
    v = kwargs["V"].detach().clone().transpose(1, 2).contiguous()
    q.requires_grad_()
    k.requires_grad_()
    v.requires_grad_()
    res = flash_attn_func(
        q, k, v, None, kwargs["IS_CAUSAL"], kwargs["sm_scale"]
    )
    dres = torch.empty_like(res)
    with tritonstd.measure_duration_torch() as dur:
        res.backward(dres)
...
@triton.jit
@tritonstd.mark_triton_compilable(inline_run_env_fn=partial(_attn_fwd_kernel_test_fn, is_fwd=False, head_first=False),
    real_kwargs={"N_CTX": 30720, "HEAD_DIM": 128, "H": 16, "dtype": torch.float16},
    raw_fn=_torch_bench_fn_bwd,
)
def _attn_bwd_bshd(Q, K, V, sm_scale,  #
              DO,  #
              DQ, DK, DV,  #
              M, D,
              # shared by Q/K/V/DO.
              stride_z, stride_tok, stride_h, stride_d,  #
              H, N_CTX,  #
              BLOCK_M1: tl.constexpr,  #
              BLOCK_N1: tl.constexpr,  #
              BLOCK_M2: tl.constexpr,  #
    ...
    off_chz = tl.cast(hid * N_CTX, tl.int64)
    adj = tl.cast(stride_h * hid, tl.int64)
    sid = tl.program_id(1)
    ...
    if (IS_CAUSAL):
        ...
        start_m = start_n + num_steps * MASK_BLOCK_M1
