...
class DraftFileStoreBackendInMemory(DraftFileStoreBackendBase):
    ...
    async def update(self, path: str, ops: list[DraftUpdateOp]) -> None:
        ...
        ops = [dataclasses.replace(o, opData=json.loads(json.dumps(o.opData))) for o in ops]
        data = self._data.get(path)
        if data is None:
            assert ops[0].op == JMESPathOpType.RootAssign, f"path {path} not exist, {ops[0]}"
        is_root_changed, root_obj = apply_draft_update_ops_to_json_with_root(data, ops)
        if is_root_changed:
...
class DraftMongoStoreBackend(DraftFileStoreBackendBase):
    ...
    async def write(self, path: str, data: Any) -> None:
        coll, key = self._get_coll_and_key(path)
        coll.replace_one({"key": key}, {"key": key, "value": data}, upsert=True)
...
def asdict_map_trace(obj, dict_factory=_default_asdict_map_trace_factory, cur_field_type: Optional[Any] = None):
    return _asdict_map_trace_inner(obj, [], dict_factory, cur_field_type=cur_field_type)
        ...
        for k, v, types in obj:
            ...
            if v is not None:
                if isinstance(store_meta, DraftStoreMapMeta):
                    ...
                    store_id = store_meta.store_id
                    storage_key = store_meta.attr_key if store_meta.attr_key else k
                    self._store_pairs.append((parts + [storage_key], v, store_id))
                    continue
...
def _analysis_model_store_meta(model_type: type[T], field_meta_dict: dict[int, FieldMeta], parent_metas: list[FieldMeta], all_store_ids: Optional[set[str]] = None):
    ...
    for field in dataclasses.fields(model_type):
        ...
        if id(field) not in field_meta_dict:
            ...
            prev_meta.contain_nested_map = isinstance(store_meta, DraftStoreMapMeta)
            prev_meta.contain_nested_store = isinstance(store_meta, DraftStoreMetaBase)
            # avoid nested check
            continue
        if isinstance(store_meta, (DraftStoreMapMeta, DraftStoreScalarMeta)):
            for parent_meta in parent_metas:
                ...
                parent_meta.contain_nested_map = isinstance(store_meta, DraftStoreMapMeta)
            ...
            if isinstance(store_meta, DraftStoreMapMeta):
                ...
                if value_type.is_dataclass_type():
                    _analysis_model_store_meta(value_type.origin_type, field_meta_dict, parent_metas + [field_meta], all_store_ids)
                else:
                ...
                for t in child_type_generator_with_dataclass(annotype.origin_type):
                    if is_annotated(t):
                        annometa = t.__metadata__
                        for m in annometa:
                            if isinstance(m, DraftStoreMetaBase):
                                raise ValueError(f"subtype of field {field.name} with type {annotype.origin_type} can't contain any store meta")
                field_meta.draft_store_meta = store_meta
            continue
        if annotype.is_dataclass_type():
            has_splitted_store |= _analysis_model_store_meta(annotype.origin_type, field_meta_dict, parent_metas + [field_meta], all_store_ids)
        else:
            # all non-dataclass field type shouldn't contain any store meta
...
def analysis_model_store_meta(model_type: type[T], all_store_ids: Optional[set[str]] = None):
    field_meta_dict: dict[int, FieldMeta] = {}
    res = _analysis_model_store_meta(model_type, field_meta_dict, [], all_store_ids)
...
def _get_relative_store_node_and_path(op: DraftUpdateOp, field_meta_dict: dict[Optional[int], FieldMeta]):
    node = op.node
    ...
    if field_meta is not None and field_meta.draft_store_meta and isinstance(field_meta.draft_store_meta, DraftStoreMapMeta):
    ...
    is_direct_mod = False
    if (op.op == JMESPathOpType.SetAttr or op.op == JMESPathOpType.ScalarInplaceOp) and op.field_id is not None:
    ...
    while relative_node_cur.children:
        ...
        if field_meta is not None:
            if isinstance(field_meta.draft_store_meta, DraftStoreMapMeta):
                ...
                path.extend([field_meta.draft_store_meta.encode_key(relative_node_cur.value), storage_key])
            ...
            if isinstance(field_meta.draft_store_meta, (DraftStoreMapMeta, DraftStoreScalarMeta)):
...
def _get_op_modify_dict_with_store(op: DraftUpdateOp, is_map_mod: bool, field_meta_dict: dict[Optional[int], FieldMeta]):
    ...
    if op.op == JMESPathOpType.SetAttr:
        ...
        if field_id in field_meta_dict:
            ...
            update_or_setattr_on_dict = isinstance(field_meta.draft_store_meta, DraftStoreMapMeta)
            if isinstance(field_meta.draft_store_meta, DraftStoreMapMeta):
...
def get_splitted_update_model_ops(root_path: str, ops: list[DraftUpdateOp], main_store_id: str, field_meta_dict: dict[Optional[int], FieldMeta]):
    ops_with_paths: dict[str, tuple[str, list[Union[DraftUpdateOp, SplitNewDeleteOp]]]] = {}
    ops_with_paths_batch: list[Union[dict[str, tuple[str, list[Union[DraftUpdateOp, SplitNewDeleteOp]]]], SplitNewDeleteOp]] = [ops_with_paths]
    for op in ops:
        node: DraftASTNode = op.node 
        paths: list[str] = []
        # convert absolute path (node) to relative path
        relative_node, paths, is_map_mod, is_direct_mod, store_id = _get_relative_store_node_and_path(op, field_meta_dict)
        if store_id is None:
        ...
        update_or_setattr_on_dict, map_items, map_store_meta, map_contains_nested = _get_op_modify_dict_with_store(op, is_map_mod, field_meta_dict)
        if update_or_setattr_on_dict:
            ...
            if op.op == JMESPathOpType.SetAttr:
                ...
                ops_with_paths_batch.append(SplitNewDeleteOp(False, all_path, value=store_id, is_remove_all_childs=True))
                ops_with_paths_batch.append(ops_with_paths)
            if map_items is not None:
                for k, v in map_items.items():
                    all_path = str(Path(root_path, *paths, map_store_meta.encode_key(k)))
                    if all_path not in ops_with_paths:
                        ops_with_paths[all_path] = (store_id, [])
                    if map_contains_nested:
                        asdict_obj = _StoreAsDict()
                        model_dict = asdict_map_trace(v, asdict_obj._asdict_map_trace_factory)
                        for p in asdict_obj._store_pairs:
                            ...
                            if all_path_nested not in ops_with_paths:
                                ops_with_paths[all_path_nested] = (store_id_nested, [])
                            new_op_item = DraftUpdateOp(JMESPathOpType.RootAssign, p[1], ROOT_NODE)
                            ops_with_paths[all_path_nested][1].append(new_op_item)
                        new_op = DraftUpdateOp(JMESPathOpType.RootAssign, model_dict, ROOT_NODE)
                    else:
                        new_op = DraftUpdateOp(JMESPathOpType.RootAssign, v, ROOT_NODE)
            ...
            if op.op == JMESPathOpType.SetAttr:
                ...
                if not _is_none_or_undefined(target):
                    ...
                    op = dataclasses.replace(op, opData={"items": [
                        (k, {})
                    ]})
        ...
        if op.op == JMESPathOpType.SetAttr:
            ...
            if field_id in field_meta_dict:
                field_meta = field_meta_dict[field_id]
                if not isinstance(field_meta.draft_store_meta, DraftStoreMapMeta):
                    ...
                    if field_meta.contain_nested_store:
                        ...
                        assert not isinstance(field_meta.draft_store_meta, DraftStoreMetaBase)
                        k = op.opData["items"][0][0]
                        v = op.opData["items"][0][1]
                        all_path = str(Path(root_path, *paths))
                        all_path_for_clear = str(Path(root_path, *paths, field_meta.name))
                        ops_with_paths = {}
                        if not ops_with_paths_batch[-1]:
                            ops_with_paths_batch.pop()
                        ops_with_paths_batch.append(SplitNewDeleteOp(False, all_path_for_clear, value=store_id, is_remove_all_childs=True))
                        ops_with_paths_batch.append(ops_with_paths)
                        asdict_obj = _StoreAsDict()
                        model_dict = asdict_map_trace(v, asdict_obj._asdict_map_trace_factory, cur_field_type=field_meta.annotype.raw_type)
                        for p in asdict_obj._store_pairs:
                            all_path_nested = str(Path(all_path, *p[0]))
                            store_id_nested = p[2]
                            if store_id_nested is None:
                            ...
                            new_op_item = DraftUpdateOp(JMESPathOpType.RootAssign, p[1], ROOT_NODE)
                            ops_with_paths[all_path_nested][1].append(new_op_item)
                        op = dataclasses.replace(op, opData={
                            "items": [(k, model_dict)]
                        })
        ...
        if not paths:
            ...
            if is_map_mod:
                ...
                if op.op == JMESPathOpType.DictUpdate:
                    ...
                    if all_path not in ops_with_paths:
                        ops_with_paths[all_path] = (store_id, [])
                    new_op = dataclasses.replace(op, op=JMESPathOpType.RootInplaceOp, node=ROOT_NODE)
                    ops_with_paths[all_path][1].append(new_op)
                    continue
                ...
                if op.op == JMESPathOpType.SetAttr:
                    if isinstance(store_meta, DraftStoreScalarMeta):
                        ...
                        new_op = dataclasses.replace(op, op=JMESPathOpType.RootAssign, opData=new_value, node=ROOT_NODE)
                        ops_with_paths[all_path][1].append(new_op)
                        continue
                    ...
                    if all_path not in ops_with_paths:
                        ops_with_paths[all_path] = (store_id, [])
                    new_op = dataclasses.replace(op, op=JMESPathOpType.RootInplaceOp, opData=op.opData, node=ROOT_NODE)
                    ops_with_paths[all_path][1].append(new_op)
                    continue
            all_path = str(Path(root_path, *paths))
            if all_path not in ops_with_paths:
...
class DraftFileStorage(Generic[T]):
    def __init__(self, root_path: str, model: T, store: Union[DraftStoreBackendBase, Mapping[str, DraftStoreBackendBase]], main_store_id: str = ""):
        ...
        self._has_splitted_store, self._field_store_meta = analysis_model_store_meta(type(model), all_store_ids)
        ...
        for field in dataclasses.fields(cur_type):
            ...
            if isinstance(store_meta, DraftStoreMapMeta):
                ...
                if not _is_none_or_undefined(cur_data[field.name]):
                    ...
                    real_data = {store_meta.decode_key(Path(k).stem): v for k, v in real_data.items()}
            ...
            if annotype.is_dataclass_type():
                ...
                if isinstance(cur_data_next, dict):
                    await self._fetch_model_recursive(annotype.origin_type, cur_data_next, parts)
    ...
    async def fetch_model(self) -> T:
        ...
        if data is None:
            ...
            return self._model
        if self._has_splitted_store:
            await self._fetch_model_recursive(type(self._model), data, [self._root_path])
    ...
    async def update_model(self, root_draft: Any, ops: list[DraftUpdateOp]):
        ...
        if not self._has_splitted_store:
            ...
            return
        ops_with_paths_batch = get_splitted_update_model_ops(self._root_path, ops, self._main_store_id, self._field_store_meta)
                ...
                for path, (store_id, ops_mixed) in ops_with_paths.items():
                    ...
                    for op_mixed in ops_mixed:
                        if isinstance(op_mixed, DraftUpdateOp):
                            ...
                            if cur_update_ops:
                                cur_update_ops = [o.to_json_update_op().to_userdata_removed() for o in cur_update_ops]
                            ...
                            if op_mixed.is_new:
                                ...
                                batch_ops.append(StoreBackendOp(path, rm_type, None))
                    if cur_update_ops:
                        batch_ops.append(StoreBackendOp(path, StoreWriteOpType.UPDATE, cur_update_ops))
