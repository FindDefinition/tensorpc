...
class PFLLibrary:
    ...
    def get_compiled_unit_inline_env(self, key: Union[str, Callable]) -> PFLInlineRunEnv:
        if not isinstance(key, str):
            key = get_module_id_of_type(key)
        if key in self._inline_env_cache:
            return self._inline_env_cache[key]
        pfl_fn = self._compiled_units[key]
        ...
        assert isinstance(inline_env, PFLInlineRunEnv), "inline run env must be PFLInlineRunEnv"
...
def _register_temp_dcls_to_std(args: list[AnnotatedArg], backend: str, global_dict: dict, external_anno: Optional[tuple[dict[str, Any], Any]] = None):
    for arg in args:
        param = arg.param
        ...
        if external_anno is not None and arg.name in external_anno[0]:
            ...
            arg_type = arg.type
        if arg_type is None:
            # arg anno may exists in external, so continue instead of raise here.
            continue 
...
class PFLParser:
    def __init__(self, backend: str = "js",
    ...
    def _parse_dep_func(self, expr: ast.expr, arg_infos_from_call: tuple[list[PFLExprInfo], dict[str, PFLExprInfo]] | None = None):
        ctx = get_parse_context_checked()
        compilable = ctx.node_to_compilable[expr]
        if compilable.uid not in self._all_compiled:
            assert compilable.uid not in self._current_compiling
            self._current_compiling.add(compilable.uid)
            func_node = self.parse_func_to_pfl_ast(compilable.func, arg_infos_from_call=arg_infos_from_call)
            self._current_compiling.remove(compilable.uid)
            self._all_compiled[compilable.uid] = func_node
        ctx.depend_compilables.append(compilable.uid)
        return compilable

    def _parse_expr_to_df_ast(self, expr: ast.expr, scope: dict[str,
                                                        PFLExprInfo]) -> PFLExpr:
        ...
        try:
            if isinstance(expr, ast.Name):
                ...
                if expr in ctx.node_to_std_item:
                    ...
                    compilable = self._parse_dep_func(expr)
                    st = self._all_compiled[compilable.uid].st
                else:
                ...
                elts = [self._parse_expr_to_df_ast(elt, scope) for elt in expr.elts]
                ...
                keys = [
                    self._parse_expr_to_df_ast(key, scope) if key is not None else None
                    for key in expr.keys
                ]
                values = [
                    self._parse_expr_to_df_ast(value, scope) for value in expr.values
                ]
                ...
                args = [self._parse_expr_to_df_ast(arg, scope) for arg in expr.args]
                kw_keys: list[str] = []
                vals: list[PFLExpr] = []
    ...
    def _parse_block_to_pfl_ast(self, body: list[ast.stmt],
                ...
                if isinstance(stmt, (ast.Assign, ast.AnnAssign)):
                    ...
                    if isinstance(tgt, ast.Name) and value is not None:
                        ...
                        for i, elt in enumerate(tgt.elts):
                            is_new_var = False
                            assert isinstance(elt, ast.Name), "assign tuple item must be Name"
                            if elt.id not in scope:
                                is_new_var = True
                            else:
                                value.st.childs[i].check_convertable(scope[elt.id],
                                                        "assign value")
                    ...
                    orelse, orelse_rinfo = self._parse_block_to_pfl_ast(stmt.orelse, private_scope_else)
                    ...
                    value = self._parse_expr_to_df_ast(stmt.iter, private_scope)
                    tgt = stmt.target
                    ...
                    target = self._parse_expr_to_df_ast(stmt.target, private_scope)
                    forbody, rinfo = self._parse_block_to_pfl_ast(stmt.body, private_scope)
                    ...
                    node = PFLExprStmt(PFLASTType.EXPR_STMT,
                                    source_loc,
                                    value=self._parse_expr_to_df_ast(stmt.value,
                                                                scope))
                    block.append(node)
    ...
    def _map_call_args_to_func_def(self, args: list[PFLExprInfo], kwargs: dict[str, PFLExprInfo], func_node: ast.FunctionDef):
        # only handle args and kwargs here.
        res: dict[str, PFLExprInfo] = {}
        arg_name_to_ast_arg: dict[str, ast.arg] = {arg.arg: arg for arg in func_node.args.args}
        for j in range(len(args)):
            ast_arg = func_node.args.args[j]
    ...
    def _parse_funcdef_to_pfl_ast(self, stmt: ast.FunctionDef, scope: dict[str, PFLExprInfo], 
            external_annos: Optional[tuple[dict[str, Any], Any]] = None,
            arg_infos_from_call: Optional[tuple[list[PFLExprInfo], dict[str, PFLExprInfo]]] = None):
                ...
                if arg.arg in arg_dict_from_call:
                    ...
                    if anno_in_ast is not None:
                        if func_meta is not None and func_meta.anno_transform is not None:
                            ...
                            assert anno_st_from_call.is_convertable(anno_in_ast)
    ...
    def parse_func_to_pfl_ast(self, 
            func: Callable,
            scope: Optional[dict[str, PFLExprInfo]] = None,
            external_anno: Optional[tuple[dict[str, Any], Any]] = None,
            arg_infos_from_call: Optional[tuple[list[PFLExprInfo], dict[str, PFLExprInfo]]] = None) -> PFLFunc:
        ...
        # TODO remove this
        _register_temp_dcls_to_std(args, backend, parse_ctx.cache._temp_dcls_dict, external_anno)
        with enter_parse_context(parse_ctx) as ctx:
            ...
            try:
                func_pfl_node = self._parse_funcdef_to_pfl_ast(func_node, scope, external_anno, arg_infos_from_call)
                func_pfl_node.uid = func_uid
                func_pfl_node.deps = ctx.depend_compilables
                func_pfl_node.backend = backend
                func_pfl_node.st.compiled_uid = func_uid
                ...
                if is_root:
                    self._all_compiled[func_uid] = func_pfl_node
    ...
    def parse_expr_to_df_ast(
        ...
        if var_scope is not None:
            ...
            #     MapAstNodeToStdlib(var_scope, PFLErrorFormatContext(expr_str_lines), backend=backend).visit(tree))
