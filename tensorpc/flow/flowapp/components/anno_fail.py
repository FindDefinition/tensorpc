...
def init_anno_fwd_v2(
        this: Callable[Concatenate[Any, P], Any],
        val: Optional[T3] = None) -> Callable[[Callable], Callable[P, T3]]:
...
class Component(Generic[T]):
    ...
    def __init__(self, prop_cls: Callable[P, T], prop_cls2: Type[T],
    ...
    def some_func(self, val: Callable[P, T], *args: P.args,
                  **kwargs: P.kwargs):
        return
    ...
    def prop_pylance_dont_work_too(self):
        ...
        @init_anno_fwd(partial(self.some_func, val=self.__prop_cls),
                       self.__props)
        def wrapper(**kwargs):
            for k, v in kwargs.items():
                setattr(self.__props, k, v)
