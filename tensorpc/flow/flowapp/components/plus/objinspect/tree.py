...
async def _get_obj_by_uid(obj, uid: str,
    ...
    res1, res2 = await _get_obj_by_uid_trace(obj, uid, checker)
    return res1[-1], res2

async def _get_obj_by_uid_trace(obj, uid: str,
                    checker: Callable[[Type], bool]) -> Tuple[List[Any], bool]:
    parts = uid.split(_GLOBAL_SPLIT)
    if len(parts) == 1:
        return [obj], True
    # uid contains root, remove it at first.
    res1, res2 = await _get_obj_by_uid_trace_resursive(obj, parts[1:], checker)
    return [obj] + res1, res2
...
async def _get_obj_by_uid_trace_resursive(
    ...
    if len(parts) == 1:
        ...
        objs, res = await _get_obj_by_uid_trace_resursive(child_obj, parts[1:], checker)
        return [child_obj] + objs, res
...
class ObjectTree(mui.FlexBox):
    ...
    async def _on_expand(self, uid: str):
        ...
        if isinstance(obj, TreeItem):
            obj_dict = await obj.get_child_desps(uid)
        else:
