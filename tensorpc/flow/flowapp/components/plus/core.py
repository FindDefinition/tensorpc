...
class ListSlider(mui.Slider, Generic[T]):
    ...
    """
    def __init__(self, label: str, callback: Callable[[T], mui._CORO_NONE], init: Optional[List[T]] = None) -> None:
        if init is None:
            init = []
        ...
        self.register_event_handler(self.__callback_key, callback, backend_only=True)
