...
class DownloadFromSSH(EditableLayoutApp):
    ...
    async def download_files(self):
        client = SSHClient(self.ssh_url.value, self.ssh_username.value, self.ssh_password.value, None)
        assert Path(self.target_loc.value).exists()
        async with client.simple_connect() as conn:
            files = self.ssh_files.value.split("\n")
            files = list(filter(lambda x: len(x), map(lambda x: x.strip(), files)))
            for f in files:
                await asyncssh.scp((conn, f), self.target_loc.value, preserve=True, recurse=True)
                print("FINISHED!", f)