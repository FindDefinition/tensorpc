...
class RemoteComponentService:
    ...
    async def set_layout_object(self, key: str, obj):
        ...
        if isinstance(obj, FlexBox):
            ...
            app: App = EditableApp(external_wrapped_obj=obj, reload_manager=reload_mgr, is_remote_component=True)
        ...
        app_obj = AppObject(app, send_loop_queue, asyncio.Event(), asyncio.Event())
    ...
    async def mount_app(self, node_uid: str, key: str, url: str, port: int, prefixes: List[str]):
        assert key in self._app_objs
        app_obj = self._app_objs[key]
        # 
        assert app_obj.mounted_app_meta is None, "already mounted"
        app_obj.mounted_app_meta = MountedAppMeta(node_uid, url, port, key, prefixes)
        app_obj.mount_ev.set()
        app_obj.app.app_storage.set_remote_grpc_url(app_obj.mounted_app_meta.url_with_port)
    ...
    def get_layout_dict(self, key: str, prefixes: List[str]):
        ...
        lay["remoteRootUid"] = UniqueTreeId.from_parts(prefixes + root_uid.parts).uid_encoded
        return lay
    ...
    async def _send_loop(self, app_obj: AppObject):
        ...
        shut_task = asyncio.create_task(app_obj.shutdown_ev.wait())
        ...
        wait_for_mount_task = asyncio.create_task(app_obj.mount_ev.wait())
        robj: Optional[tensorpc.AsyncRemoteManager] = None
        wait_tasks: List[asyncio.Task] = [shut_task, send_task, wait_for_mount_task]
        while True:
            ...
            if wait_for_mount_task in done:
                ...
                robj = tensorpc.AsyncRemoteManager(app_obj.mounted_app_meta.url_with_port)
            ...
            if app_obj.mounted_app_meta is None or robj is None:
                ...
                send_task = asyncio.create_task(app_obj.send_loop_queue.get())
                wait_tasks: List[asyncio.Task] = [shut_task, wait_for_mount_task, send_task]
            ...
            for _ in range(retry_cnt):
                try:
                    await self._send_grpc_event_large(
                        ev, robj, app_obj.mounted_app_meta.prefixes, app,
                        timeout=10)
                    succeed = True
                    break
    ...
    async def _send_grpc_event_large(self, ev: AppEvent,
        ...
        for ui_ev in ev.type_to_event.values():
            if isinstance(ui_ev, UpdateComponentsEvent):
                ...
                ui_ev.remote_component_all_childs = patch_uid_list_with_prefix(list(comp_dict.keys()), prefixes)
        ev_dict = ev.to_dict()
        ev_dict = patch_unique_id(ev_dict, prefixes)
        # print("APP EVENT", ev_dict)
        return await robj.chunked_remote_call(serv_names.APP_RELAY_APP_EVENT_FROM_REMOTE,
                                              ev_dict, rpc_timeout=timeout)
