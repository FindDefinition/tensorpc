...
class AsyncSSHTerminal(Terminal):
    def __init__(self, url: str, username: str, password: str, init_data: Optional[Union[bytes, str]] = None, 
                connect_when_mount: bool = False) -> None:
    ...
    async def connect(self, event_callback: Optional[Callable[[SSHEvent], None]] = None):
        await self.connect_with_new_info(self._client.url, self._client.username, self._client.password, event_callback)
    ...
    async def connect_with_new_info(self, url: str, username: str, password: str, event_callback: Optional[Callable[[SSHEvent], None]] = None):
        ...
        if self._init_size is not None:
            await self._cur_inp_queue.put(SSHRequest(SSHRequestType.ChangeSize, [self._init_size["width"], self._init_size["height"]]))
        self._ssh_task = asyncio.create_task(
            self._client.connect_queue(self._cur_inp_queue,
                                      partial(self._handle_ssh_queue, user_callback=event_callback),
    ...
    async def _handle_ssh_queue(self, event: SSHEvent, user_callback: Optional[Callable[[SSHEvent], Any]] = None):
