...
class ComputeFlowDrafts:
    ...
    def get_node_drafts(self, node_id: str):
        ...
        code_language = D.where(selected_node.node_type == ComputeNodeType.COMPUTE, "python", "markdown")
        return ComputeFlowNodeDrafts(selected_node, node_state_draft, code_draft, code_path_draft, code_language)
...
def get_code_drafts(root_draft: ComputeFlowModelRoot, node_draft: ComputeFlowNodeModel):
    code_draft_may_module_id = D.where(node_draft.module_id != "", root_draft.path_to_code[root_draft.module_id_to_code_info[node_draft.module_id].path].code, node_draft.code, return_type=str) # type: ignore
    code_draft = D.where(node_draft.code_key != None, root_draft.shared_node_code[node_draft.code_key], code_draft_may_module_id, return_type=str) # type: ignore
    
    code_path_draft = D.where(node_draft.code_key != None, 
        D.literal_val("tensorpc://flow/shared/%s") % node_draft.code_key, 
        D.where(
            node_draft.module_id != "", root_draft.module_id_to_code_info[node_draft.module_id].path, 
            D.literal_val("tensorpc://flow/dynamic/%s") % node_draft.id, return_type=str),
...
def get_compute_flow_drafts(root_draft: ComputeFlowModelRoot):
    ...
    is_not_subflow_node_selected = D.logical_or(cur_model_draft.selected_node == None, selected_node.type != ComputeNodeType.SUBFLOW.value)
    ...
    selected_node_detail_type = D.where(selected_node == None, DetailType.NONE.value, 
        D.where(
            selected_node.type == ComputeNodeType.SUBFLOW.value, DetailType.SUBFLOW.value, DetailType.USER_LAYOUT.value)
    , return_type=int) # type: ignore
    
    show_editor = D.logical_and(root_draft.settings.isEditorVisible, D.logical_or(selected_node.type == ComputeNodeType.MARKDOWN.value, selected_node.type == ComputeNodeType.COMPUTE.value))
    node_has_detail = D.logical_or(D.logical_and(selected_node_detail_type != DetailType.SUBFLOW.value, selected_node.has_detail), selected_node_detail_type == DetailType.SUBFLOW.value)
    show_detail = D.logical_and(root_draft.settings.isPreviewVisible, D.logical_and(selected_node_detail_type != DetailType.NONE.value, node_has_detail))
    return ComputeFlowDrafts(root_draft, cur_model_draft, prev_path_draft, preview_model_draft, selected_node, code_draft, code_path_draft, selected_node_detail_type,
