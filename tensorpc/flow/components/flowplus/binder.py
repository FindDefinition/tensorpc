...
class ComputeFlowBinder:
    def __init__(self, flow_comp: Flow, flow_comp_preview: Flow, drafts: ComputeFlowDrafts):
        self.flow_comp = flow_comp
        self.flow_comp_preview = flow_comp_preview
        self.drafts = drafts


    def _get_preview_flow_uid(self, path_draft, dm_comp: mui.DataModel[ComputeFlowModelRoot]):
        path = D.evaluate_draft(path_draft, dm_comp.model)
        if path is None:
            return "root"
        return UniqueTreeIdForTree.from_parts(path).uid_encoded

    def to_ui_node(self, node_model: ComputeFlowNodeModel, dm_comp: mui.DataModel[ComputeFlowModelRoot]) -> Node:
        if node_model.node_type == ComputeNodeType.COMPUTE:
            ...
            dm_comp.install_draft_change_handler(draft.code, partial(self._handle_node_draft_change, wrapper=wrapper, draft=draft), installed_comp=wrapper)
    ...
    def bind_flow_comp_with_datamodel(self, dm_comp: mui.DataModel):
        binder = models.flow.BaseFlowModelBinder(
            self.flow_comp, dm_comp.get_model, 
        ...
        preview_binder = models.flow.BaseFlowModelBinder(
            self.flow_comp_preview, dm_comp.get_model, 
            self.drafts.preview_model, 
            partial(self.to_ui_node, dm_comp=dm_comp),
            flow_uid_getter=partial(self._get_preview_flow_uid, path_draft=self.drafts.preview_path, dm_comp=dm_comp))
        binder.bind_flow_comp_with_base_model(dm_comp, self.drafts.cur_model.selected_node)
        preview_binder.bind_flow_comp_with_base_model(dm_comp, self.drafts.preview_model.selected_node)
