...
function Global:Prompt() {
	...
	if ($Global:__LastHistoryId -ne -1) {
		if ($LastHistoryEntry.Id -eq $Global:__LastHistoryId) {
			...
			$Result += "$([char]0x1b)]784;D`a"
		}
		else {
			# Command finished exit code
			# OSC 784 ; D [; <ExitCode>] ST
			$Result += "$([char]0x1b)]784;D;$FakeCode`a"
		}
	}
	# Prompt started
	# OSC 784 ; A ST
	$Result += "$([char]0x1b)]784;A`a"
	# Current working directory
	# OSC 784 ; <Property>=<Value> ST
	$Result += if ($pwd.Provider.Name -eq 'FileSystem') { "$([char]0x1b)]784;P;Cwd=$(__VSCode-Escape-Value $pwd.ProviderPath)`a" }
	...
	# Prompt
	# OSC 784 ; <Property>=<Value> ST
	$Result += "$([char]0x1b)]784;P;Prompt=$(__VSCode-Escape-Value $OriginalPrompt)`a"

	# Write command started
	$Result += "$([char]0x1b)]784;B`a"
...
if (Get-Module -Name PSReadLine) {
	...
	function Global:PSConsoleHostReadLine {
		...
		# Command line
		# OSC 784 ; E ; <CommandLine?> ; <Nonce?> ST
		$Result = "$([char]0x1b)]784;E;"
...
if ($PSVersionTable.PSVersion -lt "6.0") {
	...
	[Console]::Write("$([char]0x1b)]784;P;IsWindows=$true`a")
}
else {
	[Console]::Write("$([char]0x1b)]784;P;IsWindows=$IsWindows`a")
}

# Set ContinuationPrompt property
$ContinuationPrompt = (Get-PSReadLineOption).ContinuationPrompt
if ($ContinuationPrompt) {
	[Console]::Write("$([char]0x1b)]784;P;ContinuationPrompt=$(__VSCode-Escape-Value $ContinuationPrompt)`a")
}
...
function Set-MappedKeyHandlers {
	...
	if ($env:VSCODE_SUGGEST -eq '1') {
		...
		$result = "$([char]0x1b)]784;CompletionsPwshCommands;commands;"
...
function Send-Completions {
	...
	$result = "$([char]0x1b)]784;Completions"
