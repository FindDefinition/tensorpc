...
* Confirm Dialog: if set, the button will show a confirm dialog when clicked. usually used to confirm a dangerous operation. this feature can't be used with ```progress``` at the same time.
...
class App:
    ...
    def my_layout(self):
        ...
        callback = lambda: md.write(":green[nuclear missle launched.]")
        return mui.VBox([
            mui.IconButton(mui.IconType.Delete, callback).prop(
                muiColor="error", 
