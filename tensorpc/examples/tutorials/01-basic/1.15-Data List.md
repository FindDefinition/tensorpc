...
**Note**: the virtualization (only render visible components in container) of ```DataFlexBox``` is disabled by default. For large number of items, you can set ```virtualized``` to override this behavior.
