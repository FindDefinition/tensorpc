...
Currently only ```mui.DataFlexBox``` and ```mui.DataGrid``` support template component.

### Basic Usage

The data used by template container must be list of dict, with a unique id for each item.

**Note**: the virtualization (only render visible components in container) of ```DataFlexBox``` is enabled by default.
